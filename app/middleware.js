...
async function CarsEmailAuthenticated() {
    const session = await auth()
    if (session?.user?.userDataCars?.carsEmail) {
        redirect(endpoints.home)
    }
...
export { Unauthenticated, Authenticated, CarsAuthenticated, CarsEmailAuthenticated }
